...
class satellitePlotData:
    ...
    def updatePlots(self, t, reference, states, ctrl):
        ...
        self.phi_ref_history.append(180.0/np.pi*reference[0])  # reference panel angle
        self.phi_history.append(180.0/np.pi*states[1])  # panel angle
        self.theta_history.append(180.0/np.pi*states[0])  # base angle
        self.torque_history.append(ctrl[0])  # torque on the base

        # update the plots with associated histories
        self.handle[0].updatePlot(self.time_history, [self.phi_history, self.phi_ref_history])
        self.handle[1].updatePlot(self.time_history, [self.theta_history])
        self.handle[2].updatePlot(self.time_history, [self.torque_history])
